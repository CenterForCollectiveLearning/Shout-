...
existsPendingTradeRequest = function(other_user_id) {
	var existing_trade = Current_trade_requests.find({$or:[{"user_id_from":Meteor.userId(), "user_id_to":other_user_id},{"user_id_from":other_user_id, "user_id_to": Meteor.userId()}]});
	if (existing_trade.count()===0) {
		return false;
	}
	return true;
};
